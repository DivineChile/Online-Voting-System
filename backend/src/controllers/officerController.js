...
function buildReadOnlySetup(positions, candidates) {
...
function buildResultsByPosition(positions, candidates, ballotItems) {
  return (positions || []).map((position) => {
    const positionCandidates = (candidates || [])
      ...
      .map((candidate) => {
        const votes = (ballotItems || []).filter(
          (item) =>
            item.position_id === position.id && item.candidate_id === candidate.id
        ).length;
    ...
    const highestVotes =
      positionCandidates.length > 0 ? positionCandidates[0].votes : 0;
...
export async function getOfficerDashboardSummary(req, res) {
  try {
    ...
    if (electionError) {
      const formattedError = handleSupabaseError(
        electionError,
        'Failed to load officer dashboard summary.'
      );
    ...
    console.error('getOfficerDashboardSummary error:', error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error while loading officer dashboard summary.',
    });
  }
}

export async function getOfficerActiveElection(req, res) {
  try {
    const now = new Date().toISOString();
    ...
    return res.status(200).json({
      ...
      data: {
        election,
        positions: buildReadOnlySetup(positions, candidates),
      },
    });
  } catch (error) {
    console.error('getOfficerActiveElection error:', error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error while loading active election.',
    });
  }
}

export async function getOfficerElectionSetup(req, res) {
  try {
    ...
    console.error('getOfficerElectionSetup error:', error);
    return res.status(500).json({
      success: false,
      message: 'Internal server error while loading election setup.',
    });
  }
}

export async function getOfficerPublishedResults(req, res) {
  try {
    const { data: election, error: electionError } = await supabaseAdmin
      .from('elections')
      .select(`
    ...
    const { data: ballotItems, error: ballotItemsError } = await supabaseAdmin
      .from('ballot_items')
      .select('id, ballot_id, position_id, candidate_id')
      .in(
        'ballot_id',
        ballotIds.length > 0
          ? ballotIds
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        positions: buildResultsByPosition(positions, candidates, ballotItems),
      },
    });
  } catch (error) {
    console.error('getOfficerPublishedResults error:', error);
    return res.status(500).json({
      success: false,
